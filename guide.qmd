...
In that script, import your own package and `unittest`, and write a simple test for the `hello()` function, using the functions supplied by `unittest`. An example could be that you test if the `hello()` function indeed does not return any object (it only prints something!). Each test is always written as a method of a test class, as shown below.
...
class TestHello(unittest.TestCase):
  ...
  def test_hello(self):
    self.assertIsNone(yourpackage.hello())
```
